...
(* build: (int*int->int) * int -> Expr 
   ...
   This function uses a random number to pick which expr to build.
...
let rec build (rand,depth) = match (rand, depth) with
	...
	| (_,d) -> let n = rand(0,7) in
		...
		| n -> buildSine (build(rand,d-1))

(* build2: (int*int->int) * int -> Expr 
   Same as build but includes the Tangent and Triple builders.
*)		
...
let rec build2 (rand,depth) = match (rand, depth) with
	...
	| (_,d) -> let n = rand(0,9) in
		...
		| 5 -> buildTimes (build(rand,d-1), build(rand,d-1))
		| 6 -> buildThresh (build(rand,d-1), build(rand,d-1), build(rand,d-1), build(rand,d-1))
		| 7 -> buildTriple (build(rand,d-1), build(rand,d-1), build(rand,d-1))
		| 8 -> buildTangent (build(rand,d-1))
		| n -> buildTangent (build(rand,d-1))
